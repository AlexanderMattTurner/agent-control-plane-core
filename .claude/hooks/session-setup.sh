...
# `go install` pins the version AND verifies every module against the Go
# checksum database (sum.golang.org) before building, so the bytes cannot be
# swapped by a compromised mirror — unlike an unpinned `curl … | sh` installer,
# which executes fetched remote code with no integrity check. It downloads
# nothing via curl/wget, so there is no artifact for check-pinned-downloads to
# flag and no exemption. Returns non-zero (without warning) when go is absent
# so the caller can fall back to the signed-apt path.
go_install_pinned() {
