...
# finding. config/review-severities.json says which severities gate — this repo
# gates on all three, matching what post-pr-review.mjs already escalates to
# REQUEST_CHANGES. Resolving the last gating thread is what
...
# a hidden `<!-- severity: … -->` marker, with the finding's leading icon as the
# fallback. post-pr-review.mjs stamps no marker here, so the ICON is the live
# signal: every posted finding body starts with `icon(f.severity)`. Which
# severities gate — and which icon each renders as — comes from
# config/review-severities.json, the same SSOT post-pr-review.mjs reads.
#
...
if [[ -z "$reviews" ]]; then
  ...
  # SSOT-derived predicate built above. A severity left out of `gating` is
  # advisory: its threads are posted and never hold the merge.
  ...
  if [[ "$count" -eq 0 ]]; then
    verdict=green
    reason="the reviewer has reviewed this PR and no unresolved thread carries a merge-gating finding"
  else
    verdict=red
