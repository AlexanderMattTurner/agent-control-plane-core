...
- Conformance refuses an `UNRENDERED_FIELDS` row that is not a set. A `null` row passed an existence check and then answered nothing at every `has`, so the adapter claimed every channel while a consumer indexing into `.has(...)` crashed.
