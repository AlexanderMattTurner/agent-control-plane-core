...
- A synthesized `unknown` probe is never vetoable. `makeEvent` refuses that state, so seeding it from a vetoable tool event built an event no `parse` can produce.
