...
| **Goose**       | **No admin lock.** `~/.config/goose/.gooseignore` is user-editable, a **local `.gooseignore` can negate the global one**, and it gates **file access only**, not tool calls broadly ([docs](https://goose-docs.ai/docs/guides/using-gooseignore/)); `GOOSE_MODE`/`permission.yaml` <!-- allow-workflow-ref: permission.yaml is Goose's own config file, not a GitHub Actions workflow --> are user config | **No** — no supported extension point to register a decision-maker (`schema-fit.md`), and the ignore file is agent-writable                                                                                                                                                                                       | **Neither reliably.** `observe_only` at best; the sandbox must carry all enforcement                                           |
